...
const ChatMessage = ({ message, setIsLoading }: ChatMessageProps) => {
  ...
  const renderElement = (element, index) => {
    ...
    switch (element.type) {
      case "message":
        return (
          <p className={`chat-message ${element.role}`} key={index}>
            {element.message}
          </p>
        );
      ...
      case "section":
        return (
          <div key={index}>
            <h2>{element.content.split(":")[0]}</h2>
            <p>{element.content.split(":")[1]}</p>
          </div>
        );
